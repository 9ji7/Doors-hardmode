...
local function GetRooms()
    ...
    for _, v in ipairs(rf:GetChildren()) do
        -- только комнаты с PrimaryPart или хотя бы одним Part
        if v:IsA("Model") then
            r[#r+1] = v
        end
    end
...
local function GetRoomNode(room)
    -- Ищем узел в следующем порядке: Door > Nodes > PrimaryPart > любой Part
    local node = room:FindFirstChild("Door") or room:FindFirstChild("Nodes")
    if node then
        if node:IsA("Model") then
            return node.PrimaryPart and node.PrimaryPart.Position or node:FindFirstChildWhichIsA("BasePart").Position
        else
            return node.Position
        end
    end
    if room.PrimaryPart then return room.PrimaryPart.Position end
    local anyPart = room:FindFirstChildWhichIsA("BasePart")
    return anyPart and anyPart.Position or Vector3.new(0, 5, 0)
end
...
-- ============================================================
-- КОНТРОЛЛЕР СПАВНА — ROBUST VERSION
-- ============================================================

-- Дебаг: печатаем содержимое ReplicatedStorage и workspace для диагностики
task.spawn(function()
    task.wait(3)
    print("[Craziness DEBUG] === ReplicatedStorage children ===")
    for _, v in ipairs(ReplicatedStorage:GetChildren()) do
        print("  RS >", v.Name, v.ClassName)
        for _, c in ipairs(v:GetChildren()) do
            print("    >", c.Name, c.ClassName, (c:IsA("ValueBase") and tostring(c.Value) or ""))
        end
    end
    print("[Craziness DEBUG] === workspace children ===")
    for _, v in ipairs(workspace:GetChildren()) do
        print("  WS >", v.Name, v.ClassName)
    end
end)

-- Функция триггера спавна — вызывается при каждом новом номере комнаты
local lastTriggeredRoom = 0
local function OnRoomChanged(val)
    val = tonumber(val) or 0
    if val <= 5 then return end
    if val <= lastTriggeredRoom then return end -- не дублировать
    lastTriggeredRoom = val
    print("[Craziness] Room trigger:", val)
    ...
    if not IR_Active then
        if RNG:NextInteger(1, 100) <= Config.IR_Chance then
            IR_Active  = true
            IR_Counter = 0
            print("[Craziness] IR activated")
        end
    end
    if IR_Active then
        IR_Counter = IR_Counter + 1
        task.spawn(function() SpawnInvertedRebound(IR_Counter == 1) end)
        if IR_Counter >= 2 then IR_Active = false; IR_Counter = 0 end
    end

    -- 🦌 Deer God
    if not DG_Active and RNG:NextInteger(1, 100) <= Config.DG_Chance then
        print("[Craziness] Deer God spawning!")
        task.spawn(function() task.wait(2); SpawnDeerGod() end)
    end

    -- 🌑 Common Sense
    if RNG:NextInteger(1, 100) <= Config.CS_Chance then
        print("[Craziness] Common Sense spawning!")
        task.spawn(function() SpawnCommonSense(5, val) end)
    end

    -- 🔴 Red Smile
    if RNG:NextInteger(1, 100) <= Config.RS_Chance then
        print("[Craziness] Red Smile spawning!")
        task.spawn(function() task.wait(1); SpawnRedSmile(Config.RS_Rebounds, val) end)
    end
end

task.spawn(function()
    -- Метод 1: стандартный ReplicatedStorage.GameData.LatestRoom
    local ok1, gameData = pcall(function()
        return ReplicatedStorage:WaitForChild("GameData", 8)
    end)
    if ok1 and gameData then
        local ok2, latestRoom = pcall(function()
            return gameData:WaitForChild("LatestRoom", 8)
        end)
        if ok2 and latestRoom then
            print("[Craziness] Found LatestRoom via GameData, current value:", latestRoom.Value)
            OnRoomChanged(latestRoom.Value) -- триггер для уже загруженной комнаты
            latestRoom.Changed:Connect(OnRoomChanged)
            return
        end
    end

    -- Метод 2: поиск любого IntValue/NumberValue с "Room" в имени в ReplicatedStorage
    print("[Craziness] GameData not found, scanning ReplicatedStorage...")
    for _, v in ipairs(ReplicatedStorage:GetDescendants()) do
        if (v:IsA("IntValue") or v:IsA("NumberValue")) and v.Name:lower():find("room") then
            print("[Craziness] Found room value:", v:GetFullName(), "=", v.Value)
            OnRoomChanged(v.Value)
            v.Changed:Connect(OnRoomChanged)
            return
        end
    end

    -- Метод 3: следим за workspace.CurrentRooms напрямую (новая комната = новый ребёнок)
    print("[Craziness] Falling back to workspace.CurrentRooms watcher...")
    local currentRooms = workspace:WaitForChild("CurrentRooms", 15)
    if currentRooms then
        print("[Craziness] Watching CurrentRooms, current count:", #currentRooms:GetChildren())
        -- Первый спавн если уже в игре
        local existing = #currentRooms:GetChildren()
        if existing > 5 then OnRoomChanged(existing) end
        currentRooms.ChildAdded:Connect(function(child)
            local num = tonumber(child.Name) or #currentRooms:GetChildren()
            print("[Craziness] New room added:", child.Name, "->", num)
            OnRoomChanged(num)
        end)
        return
    end

    -- Метод 4: polling — каждые 2 секунды проверяем количество комнат
    print("[Craziness] All methods failed, using polling fallback...")
    while true do
        task.wait(2)
        local rf = workspace:FindFirstChild("CurrentRooms")
        if rf then
            local count = #rf:GetChildren()
            if count > lastTriggeredRoom then
                OnRoomChanged(count)
            end
        end
    end
